...
function countSel(lst) {
    const elements = lst.length
    const differ = [...new Set(lst)].length
    let count = {}
    let res = []
    ...
    let unique = lst.filter(function (el) {
        return count[el] === 1
    }).length

    let maxVal = Object.values(count)
    const valMax = Math.max(...maxVal)

    const keyMaxVal = Object.keys(count).filter(key => count[key] === valMax).map(Number).sort((a, b) => a - b)

    res.push(elements, differ, unique, [keyMaxVal, valMax])
    return res
}